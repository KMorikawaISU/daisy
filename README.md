...
> Simplest setting: **\[Internal\] x1 ~ N(0,1)**, **\[External\] x1 ~
> N(0.5,1)**, **x2 ~ Bern(0.5)**, \*\*y \| x ~ N(0.5\*x1 − x2, 1)**,  
> Sample sizes: \[internal\] **n = 200**, \[external\] **n1 = 2000**.  
> This chunk is **not executed\*\* to keep the README light
> (`eval = FALSE`).
...
- **First-step moments**: `MU_int` / `MU_ext` must be provided
  **without** an intercept.  
- **Second step** uses KL (fixed). For numerical compatibility with the
  original implementation, the second-step weights are built from the
