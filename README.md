...
# MU_int: DO NOT include a constant column (intercept is added internally)
MU_int <- cbind(x1_int, x1_int^2, x2_int)

# MU_ext: external moments corresponding to MU_int (no leading 1)
MU_ext <- c(mean(x1_ext), mean(x1_ext^2), mean(x2_ext))
...
# Compact report (lm-like)
summary(fit)
# Example (illustrative):
# Summary of eb_est object
#   model divergence    r   D1 D1_lt_1   D2 D2_lt_1 Entropy2    est
#       1        KL    1 0.72    TRUE 0.95    TRUE    12.31  -0.10
#       2        LW 0.10 1.08   FALSE 0.90    TRUE    12.85  -0.09
# Selected (best) model index: 2
...
- **Input convention**: `MU_int` must **not** include constant columns
  (e.g., a column of 1s). If a constant is supplied, it is dropped with
  a warning; an intercept is added internally by `EB_est()`.
