...
> **y | x ~ N(0.5*x1 - x2, 1)**, internal **n = 200**, external summaries **n1 = 2000**.  
> To keep the README light, this chunk is **not executed** (`eval = FALSE`).
...
- Step-2 uses KL (fixed). For numerical compatibility with the original implementation, the second-step weights are built from the **first-step** linear predictor.
- **Input convention**: `MU_int` must **not** include constant columns (e.g., a column of 1s). If a constant is supplied, it is dropped with a warning; an intercept is added internally by `EB_est()`.
